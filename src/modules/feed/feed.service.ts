...
export class FeedService {
    ...
    async getFeed(userId: string, page: number, limit: number) {
        ...
        const friendIds = friendships
            ...
            .map((friendship) => friendship.userAId === userId ? friendship.userBId : friendship.userAId);

        if (friendIds.length === 0) {
            return [];
        }

        const catches = await this.feedRepository.findCatchesByUserIds(friendIds, page, limit);
